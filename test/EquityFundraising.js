...
describe("EquityFundraising", function () {
    ...
    describe("create fundraising", function () {
        ...
        it("should invest in new fundraising", async function () {
            ...
            const tokenDecimals = await investmentToken.decimals(); // Number
            const chainlinkPrice = await equityFundraising.getChainlinkDataFeedLatestAnswer(investmentToken); // BigNumber
            const chainlinkDecimals = 18; // Chainlink price decimals

            let investmentAmountConverted = (investmentAmount) * Number(chainlinkPrice) / (10) ** (chainlinkDecimals - Number(tokenDecimals));
            ...
            await expect(await investmentToken.balanceOf(user1.address)).to.equal(investorBalance)
        ...
        it("should close campaign with adjusted fees and margins", async function () {
            ...
            await equityFundraising.setCuratorsContractAddress(forcefiStaking.getAddress())
