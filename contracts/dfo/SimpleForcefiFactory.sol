...
interface IFundraisingFactory{
    ...
    function createEquityFundraising(Vesting.FundraisingData memory, address[] memory, uint, address) external returns(address);
...
contract SimpleForcefiFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createEquityFundraising(
        ...
        uint _attachedERC20AddressLength,
        address _referralAddress) public{
        ...
        for(uint i=0; i<numOfFundraisings; i++){
            address fundraisingAddress = deployFundraising(_fundraisingData[i], _attachedERC20Address, _attachedERC20AddressLength, _referralAddress);
    ...
    function deployFundraising(Vesting.FundraisingData memory _fundraisingData, address [] memory _attachedERC20Address, uint _attachedERC20AddressLength, address _referralAddress) internal virtual returns(address){
        return IFundraisingFactory(fundraisingFactoryAddress).createEquityFundraising(_fundraisingData, _attachedERC20Address, _attachedERC20AddressLength, _referralAddress);
    }
