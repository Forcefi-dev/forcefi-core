...
abstract contract ForcefiOptionBase is Ownable {
    ...
    uint internal constant monthlyTimestamp = 100;
    uint internal constant dailyTimestamp = 100;
    ...
    event Executed(uint id, uint marketPrice);
    ...
    function whitelistMinters(address _contractAddress) public {
    ...
    function execute(uint _id, bool _buyout, address _erc20TokenAddress) public virtual {
        ...
        if(op.strikePrice >= marketPrice && _buyout){
            ...
            IDFOToken(tokenAddress).transfer(msg.sender, amount);
        } else {
            IDFOToken(tokenAddress).burn(amount / 4);
        }
        op.executed = true;
    ...
    function getOptionTokenAmount(uint _id) internal view virtual returns(uint){}
    ...
    function changeOwnership(uint _optionIdx, address _erc20TokenAddress) public {
        ...
        IDFOToken(_erc20TokenAddress).transferFrom(msg.sender, option.beneficiar, option.saleAmount);
